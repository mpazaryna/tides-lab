...
import type { MobileToolMetadata, MobileToolExecutionResult } from "../services/ToolRegistryService";
import type {
  Tide,
  EnergyUpdate,
  TaskLinksResponse,
...
interface MCPContextType extends MCPState {
  ...
  validateToolParams: (toolName: string, params: Record<string, any>) => any;
  executeToolDirect: (toolName: string, params?: Record<string, any>) => Promise<MobileToolExecutionResult>;
...
export function MCPProvider({ children }: MCPProviderProps) {
  ...
  const getAvailableToolNames = useCallback((): string[] => {
    return mcpService.getAvailableToolNames();
  }, []);

  const getToolsByCategory = useCallback((category?: string) => {
    return mcpService.getToolsByCategory(category);
  }, []);

  const getToolMetadata = useCallback((toolName: string) => {
    return mcpService.getToolMetadata(toolName);
  }, []);

  const findToolByPartialName = useCallback((partialName: string) => {
    return mcpService.findToolByPartialName(partialName);
  }, []);

  const validateToolParams = useCallback((toolName: string, params: Record<string, any>) => {
    return mcpService.validateToolParams(toolName, params);
  }, []);

  const executeToolDirect = useCallback(async (toolName: string, params: Record<string, any> = {}) => {
    LoggingService.info(
      "MCPContext",
      "Executing tool via direct access",
      { toolName, params },
      "MCP_CONTEXT_DIRECT_001"
    );

    return mcpService.executeToolDirect(toolName, params);
  }, []);

  const getRegistryStats = useCallback(() => {
    return mcpService.getRegistryStats();
  }, []);
