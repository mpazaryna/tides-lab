...
import { EnergyChartData, EnergyDataPoint, energyLevelToNumber } from '../types/charts';
  ...
  const fetchEnergyData = useCallback(async () => {
    ...
    try {
      ...
      if (reportResult.success && reportResult.report) {
        ...
        if (report.energy_progression && Array.isArray(report.energy_progression)) {
          report.energy_progression.forEach((energyLevel: any, index: number) => {
            // Create timestamp for each point (spread over recent time)
            const now = Date.now();
            const timeOffset = (report.energy_progression.length - 1 - index) * 2 * 60 * 60 * 1000; // 2 hours apart
      ...
      loggingService.error('EnergyData', 'Error fetching energy data', { error });
      setChartData({
        points: [],
        loading: false,
