...
export const useDailyTide = (): UseDailyTideReturn => {
  ...
  const initializeDailyTide = useCallback(async () => {
    if (!isConnected) {
      loggingService.debug("useDailyTide", "Not connected, skipping initialization");
      return;
    }
    ...
    try {
      ...
      if (result.success && result.tide) {
        ...
        loggingService.info("useDailyTide", 
          result.created ? "Created new daily tide" : "Retrieved existing daily tide", 
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to initialize daily tide";
      setError(errorMessage);
      loggingService.error("useDailyTide", "Failed to initialize daily tide", { error: errorMessage });
    } finally {
      setLoading(false);
    }
  }, [isConnected]);
  ...
  const renameDailyTide = useCallback(async (newName: string) => {
    if (!dailyTide) {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to rename tide";
      loggingService.error("useDailyTide", "Failed to rename daily tide", { error: errorMessage });
      throw err;
    }
  }, [dailyTide]);
