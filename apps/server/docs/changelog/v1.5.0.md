...
**⚠️ Important: These prompts generate rich context for AI models - they DO NOT perform analysis themselves. The generated prompt content must be sent to an AI model for actual analysis.**
...
const ProductivityDashboard = ({ tides }) => {
  ...
  return (
    <ScrollView>
      <Button title="Get Insights" onPress={analyzeAllTides} />
      {loading && <ActivityIndicator />}
      
      {Object.entries(insights).map(([tideId, insight]) => (
        <Card key={tideId}>
          <Text style={styles.title}>Analysis for {tideId}</Text>
          <Text>{insight}</Text>
        </Card>
      ))}
    </ScrollView>
...
```
src/prompts/
├── registry.ts        # All prompt templates with metadata
├── loader.ts         # Template processing utilities  
├── analyze-tide.md   # Documentation template
├── productivity-insights.md
├── optimize-energy.md
├── team-insights.md
└── custom-tide-analysis.md
```
...
- ✅ **tides-001 (Development)** - https://tides-tides-001.mpazbot.workers.dev
- ✅ **tides-002 (Staging)** - https://tides-tides-002.mpazbot.workers.dev
- ✅ **tides-003 (Production)** - https://tides-tides-003.mpazbot.workers.dev
